...
public class OpenAiLlmClient : ILlmClient
{
    ...
    public async Task<LlmResponse> CompleteAsync(LlmRequest request, CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 1; ; attempt++)
        {
            ...
            {
                throw new HttpRequestException($"OpenAI API error {(int)httpResponse.StatusCode}: {responseBody}", null, httpResponse.StatusCode);
            }
    ...
    /// <summary>
    /// Serialises a message carrying images. "detail": "high" is deliberate — the documentation
    /// images are dense tables, and the low-detail tier downsamples them past readability.
    /// </summary>
