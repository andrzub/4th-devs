...
/// <summary>
/// The function-calling loop. Tool calls run one after another rather than concurrently: every
/// call changes the state of a stateful machine and spends a metered request, and the guard's
/// picture of that state only holds while commands arrive in a known order.
/// </summary>
    ...
    int maxNudges = 3)
{
    ...
    private async Task<(string Result, string? Abort)> InvokeAsync(ToolCall call, bool alreadySettled, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex) when (ex is HubBudgetExceededException or ShellBudgetExceededException or ShellBanException)
        {
            // Nothing the model can do about these, so the run ends instead of retrying.
