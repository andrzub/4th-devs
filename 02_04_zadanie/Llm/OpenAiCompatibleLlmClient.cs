...
/// chat-completions protocol. One instance per role: the agent model and the cheap
/// scanner model are separate instances of this class with different settings.
/// No third-party SDK: only HttpClient and System.Text.Json.
...
public class OpenAiCompatibleLlmClient : ILlmClient
{
    ...
    public OpenAiCompatibleLlmClient(string providerName, LlmProviderSettings settings)
    {
        if (string.IsNullOrWhiteSpace(settings.ApiKey))
            throw new InvalidOperationException($"Missing API key for '{providerName}'. Set it in appsettings.Development.json.");
    ...
    public string ModelName => _defaultModel;
    ...
    public async Task<LlmResponse> CompleteAsync(LlmRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // The gate only enforces the minimum spacing between requests. When no spacing is configured
        // (the researcher role), requests may overlap, so delegated agents really do run in parallel.
        var throttled = _minRequestInterval > TimeSpan.Zero;
        if (throttled)
    ...
    /// <summary>
    /// Some providers (Gemini) return no Retry-After header; the suggested wait lives in the
    /// error body as <c>"retryDelay": "41s"</c> (google.rpc.RetryInfo). A small safety margin is added.
    /// </summary>
